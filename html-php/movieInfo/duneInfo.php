...
<html>
    ...
    <body style="background-color: #030e16; color: white;">
        ...
        <div class="container-trui" id="header-parent-container">
            ...
            <div class="picture">
                <img src="../../images/movies/dune-landscape.jpg" alt="movie image" class="galery"/><br/>
            </div>

                <div class="contain">   
                    <?php
                        $query = "SELECT * FROM Products WHERE productName = 'Dune'" or die("Error: there was an error fetching the image");
                        $result = mysqli_query($link, $query) or die("Could not execute the query");
                                            
                        $row = mysqli_fetch_array($result);
                        
                        echo("<div><img src=\"../" . $row['image'] . "\" alt=\"movie image\" class=\"movie-image\"/><br/></div>");
                    ?>
                    <div class="details">
                        <?php
                            //Retrieving the category name of the product
                            $query2 = "SELECT categoryName FROM `category` LEFT JOIN productcategory ON category.categoryID = productcategory.categoryID LEFT JOIN products ON products.productID = productcategory.productID WHERE productName = \"Dune\";" or die("Error: there was an error fetching the image");
                            $result2 = mysqli_query($link, $query) or die("Could not execute the query");
                            $row2 = array();
                            ...
                            echo("<p class=\"pg\"><b>" . $row['parentalGuidance'] . "</b></p>&nbsp;&nbsp;");
                            ...
                            echo("<span>Genre: " . $row2['categoryName'] . "</span>");
                            echo("<br/>");
                            echo("<span>Actors: Zendaya, Rebecca Ferguson, Timothée Chalamet...</span><br/>");
                            echo("<span>Director: Denis Villeneuve</span><br/>");
                            echo("<span>Release date: " . $row['releaseDate'] . "</span>");
                            
                        ?>
                    </div>
        
                    <div class="extra-details">
                        <?php echo("<p class=\"amount\"><b>" . $row['price'] . "$</b></p>") ?>
                        <button class="button">Add to cart</button>
                        <button class="button">Buy now</button>
                        <hr/>
                        <a href="https://youtu.be/n9xhJrPXop4" target="_blank" title="Click to watch movie trailer"><button class="button">Trailer</button></a>
                    </div>
                </div>
                ...
                <!--<div class="container">
                    <a href="../../html-php/movieInfo/inceptionInfo.html">
